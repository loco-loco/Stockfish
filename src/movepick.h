...
struct Stats {
  ...
  void update_fm(Piece pc, Square to, Value v) {

    if (abs(int(v)) >= 324)
        return;

    table[pc][to] -= table[pc][to] * abs(int(v)) / 1024;
    table[pc][to] += int(v) * 32;
  }
  void update_cm(Piece pc, Square to, Value v) {

    if (abs(int(v)) >= 324)
        return;

    table[pc][to] -= table[pc][to] * abs(int(v)) / 768;
    table[pc][to] += int(v) * 32;
  }
